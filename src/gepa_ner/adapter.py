...
class NERSyntheticAdapter:
    ...
    def evaluate(
        ...
        self._log_wandb(num_valid, len(gen_traces), scores, result, delta)
    ...
    def _log_wandb(
        ...
        try:
            ...
            if wandb.run is None:
                return
            wandb.log({
                "gepa/eval_count": self._eval_count,
                "gepa/num_augmented": num_valid,
                "gepa/delta_relaxed_f1": delta,
                "gepa/baseline_relaxed_f1": self._baseline_relaxed_f1,
                "gepa/f1_strict": result.f1,
                "gepa/f1_relaxed": result.relaxed_f1,
                "gepa/precision_relaxed": result.relaxed_precision,
                "gepa/recall_relaxed": result.relaxed_recall,
                "gepa/augmentation_success_rate": num_valid / total_calls if total_calls else 0.0,
            })
        except ImportError:
            pass
        self._eval_count += 1
